...
import {AbstractBaseLoader, type LoaderValueResult, type OverrideKeyMap, VariableLookupError} from '@luolapeikko/core-env';
 ...
 * @since v0.0.1
 */
export class DockerSecretsConfigLoader<OverrideMap extends OverrideKeyMap = OverrideKeyMap> extends AbstractBaseLoader<
		...
		return Result.asyncTupleFlow(this.getOptions(), async (options) => {
			...
			if (!seen) {
				if (!existsSync(filePath)) {
					if (!options.isSilent) {
						return Err(new VariableLookupError(lookupKey, `ConfigVariables[${this.loaderType}]: ${lookupKey} from ${filePath} not found`));
					}
			...
			return Ok({path: filePath, value: await valuePromise});
		});
	}
